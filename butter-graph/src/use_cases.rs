...
fn org_chart() -> Result<(), GraphErr> {
    ...
    let employees = g.add_fn(|b| {
        b.add_call(
            Op::VecConcat,
            vec![shane.into(), alice.into(), myk.into(), jay.into()],
        );
    });
    ...
    let reports_to = g.add_fn(|b| {
        b.add_call(
            Op::VecConcat,
            vec![
                alice_reports_to_shane.into(),
                myk_reports_to_shane.into(),
                jay_reports_to_shane.into(),
            ],
        );
    });
        ...
        b.add_call(
            ...
            vec![
                Value::Function(Function::build(|b| {
                    b.add_call(
                        Op::Accessor,
                        vec![FunctionOp::Binding(0), "employee_id".into()],
                    );
                }))
                .into(),
                employees.into(),
