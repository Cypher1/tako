...
complementTests = testGroup "Complement tests" $
  [ testCase "Complement 10 = -11" $ do
    (pRet, -11)`elem`(exec (B Not pa pRet) [(pa, 10)]) @?= True
  , testCase "Complement -11 = 10" $ do
...
addTests :: TestTree
addTests = testGroup "Add tests" $
  [ testCase "Add 10+3 =" $ do
    (pRet, 13)`elem`(exec (T Add pa pb pRet) [(pa, 10), (pb, 3)]) @?= True
  , testCase "Sub 10-3 =" $ do
