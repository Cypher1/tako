...
template <typename T> struct has_insertion_operator {
  ...
  static const bool value =
      sizeof(decltype(testStreamable(s << t))) == sizeof(yes);
};
} // namespace has_insertion_operator_impl
...
public:
  ...
  template <typename T> DOCTEST_NOINLINE ContextBuilder &operator<<(T &in) {
    Capture<T> temp(&in);
  ...
  template <typename T> ContextBuilder &operator<<(const T &&) {
    static_assert(deferred_false<T>::value,
                  "Cannot pass temporaries or rvalues to the streaming "
                  "operator because it "
...
int registerReporter(const char *name, int priority) {
  detail::registerReporterImpl(name, priority,
                               detail::reporterCreator<Reporter>);
  return 0;
}
} // namespace doctest
...
template <class T, T x> constexpr T to_lvalue = x;
    ...
    if (!doctest::getContextOptions()->no_throw) {                             \
      ...
      try {                                                                    \
        DOCTEST_CAST_TO_VOID(expr)                                             \
          ...
          const doctest::detail::remove_const<                                 \
...
struct ConsoleReporter : public IReporter {
  ...
  void test_run_end(const TestRunStats &p) override {
    ...
    if (opt.no_skipped_summary == false) {
      ...
      s << (numSkipped == 0 ? Color::None : Color::Yellow) << std::setw(6)
    ...
    s << Color::Cyan << "[doctest] " << Color::None
      ...
      << std::setw(6) << (p.numAsserts - p.numAssertsFailed) << " passed"
      << Color::None << " | "
...
bool parseOptionImpl(int argc, const char *const *argv, const char *pattern,
                     String *value) {
  // going from the end to the begining and stopping on the first occurance from
  // the end
