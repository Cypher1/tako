...
fn test_with_expectation(expected: TestResult, options: Vec<&str>) {
    ...
    match (result, expected) {
        ...
        (Ok(result), OutputFile(gold)) => {
            ...
            let golden = read.expect(format!("golden file {} could not be read", gold).as_str());
            assert_eq!(
                golden,
                format!("{}{}", stdout.join(""), result)
            );
...
#[test]
fn compile_1_plus_2() {
  test_with_expectation(OutputFile("goldens/examples_1_plus_2.cc".to_string()), vec!["examples/1_plus_2.tk"]);
}

#[test]
fn compile_arguments() {
  test_with_expectation(OutputFile("goldens/examples_arguments.cc".to_string()), vec!["examples/arguments.tk"]);
}

#[test]
fn compile_code_reuse() {
  test_with_expectation(OutputFile("goldens/examples_code_reuse.cc".to_string()), vec!["examples/code_reuse.tk"]);
}

#[test]
fn compile_comment() {
  test_with_expectation(OutputFile("goldens/examples_comment.cc".to_string()), vec!["examples/comment.tk"]);
}

#[test]
fn compile_div() {
  test_with_expectation(OutputFile("goldens/examples_div.cc".to_string()), vec!["examples/div.tk"]);
}

#[test]
fn compile_empty_def_args() {
  test_with_expectation(OutputFile("goldens/examples_empty_def_args.cc".to_string()), vec!["examples/empty_def_args.tk"]);
}

#[test]
fn compile_hello_name() {
  test_with_expectation(OutputFile("goldens/examples_hello_name.cc".to_string()), vec!["examples/hello_name.tk"]);
}

#[test]
fn compile_higher_order() {
  test_with_expectation(OutputFile("goldens/examples_higher_order.cc".to_string()), vec!["examples/higher_order.tk"]);
}

#[test]
fn compile_ignored_let() {
  test_with_expectation(OutputFile("goldens/examples_ignored_let.cc".to_string()), vec!["examples/ignored_let.tk"]);
}

#[test]
fn compile_lambda() {
  test_with_expectation(OutputFile("goldens/examples_lambda.cc".to_string()), vec!["examples/lambda.tk"]);
}

#[test]
fn compile_multi_comment() {
  test_with_expectation(OutputFile("goldens/examples_multi_comment.cc".to_string()), vec!["examples/multi_comment.tk"]);
}

#[test]
fn compile_multi_comment_nested() {
  test_with_expectation(OutputFile("goldens/examples_multi_comment_nested.cc".to_string()), vec!["examples/multi_comment_nested.tk"]);
}

#[test]
fn compile_neg() {
  test_with_expectation(OutputFile("goldens/examples_neg.cc".to_string()), vec!["examples/neg.tk"]);
}

#[test]
fn compile_nested() {
  test_with_expectation(OutputFile("goldens/examples_nested.cc".to_string()), vec!["examples/nested.tk"]);
}

#[test]
fn compile_nested_as_function() {
  test_with_expectation(OutputFile("goldens/examples_nested_as_function.cc".to_string()), vec!["examples/nested_as_function.tk"]);
}

#[test]
fn compile_nested_explicit() {
  test_with_expectation(OutputFile("goldens/examples_nested_explicit.cc".to_string()), vec!["examples/nested_explicit.tk"]);
}

#[test]
fn compile_non_overlapping_anons() {
  test_with_expectation(OutputFile("goldens/examples_non_overlapping_anons.cc".to_string()), vec!["examples/non_overlapping_anons.tk"]);
}

#[test]
fn compile_not() {
  test_with_expectation(OutputFile("goldens/examples_not.cc".to_string()), vec!["examples/not.tk"]);
}

#[test]
fn compile_optional_semis() {
  test_with_expectation(OutputFile("goldens/examples_optional_semis.cc".to_string()), vec!["examples/optional_semis.tk"]);
}

#[test]
fn compile_order_of_ops() {
  test_with_expectation(OutputFile("goldens/examples_order_of_ops.cc".to_string()), vec!["examples/order_of_ops.tk"]);
}

#[test]
fn compile_paren() {
  test_with_expectation(OutputFile("goldens/examples_paren.cc".to_string()), vec!["examples/paren.tk"]);
}

#[test]
fn compile_pow() {
  test_with_expectation(OutputFile("goldens/examples_pow.cc".to_string()), vec!["examples/pow.tk"]);
}

#[test]
fn compile_pow_twice() {
  test_with_expectation(OutputFile("goldens/examples_pow_twice.cc".to_string()), vec!["examples/pow_twice.tk"]);
}
...
#[test]
fn compile_simple() {
  test_with_expectation(OutputFile("goldens/examples_simple.cc".to_string()), vec!["examples/simple.tk"]);
}

#[test]
fn compile_simple_call() {
  test_with_expectation(OutputFile("goldens/examples_simple_call.cc".to_string()), vec!["examples/simple_call.tk"]);
}

#[test]
fn compile_sub() {
  test_with_expectation(OutputFile("goldens/examples_sub.cc".to_string()), vec!["examples/sub.tk"]);
}

#[test]
fn compile_three_vars() {
  test_with_expectation(OutputFile("goldens/examples_three_vars.cc".to_string()), vec!["examples/three_vars.tk"]);
}

#[test]
fn compile_tmp() {
  test_with_expectation(OutputFile("goldens/examples_tmp.cc".to_string()), vec!["examples/tmp.tk"]);
}

#[test]
fn compile_x_plus_1() {
  test_with_expectation(OutputFile("goldens/examples_x_plus_1.cc".to_string()), vec!["examples/x_plus_1.tk"]);
}
...
#[test]
fn hello_name() {
  test_with_expectation(OutputFile("goldens/examples_hello_name.txt".to_string()), vec!["--run", "examples/hello_name.tk", "--", "Peanut", "Josh"]);
}
...
fn missing_arguments() {
  test_with_expectation(Error, vec!["--run", "counter_examples/missing_arguments.tk"]);
}
...
fn nested_explicit_side_effect() {
  test_with_expectation(Error, vec!["--run", "counter_examples/nested_explicit_side_effect.tk"]);
}
...
#[test]
fn override_print() {
  test_with_expectation(OutputFile("goldens/examples_override_print.txt".to_string()), vec!["--run", "examples/override_print.tk", "--"]);
}
