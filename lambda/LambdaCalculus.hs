...
printTm ctx t = case t of
  TmAbs _ x t1 ->
    let (ctx', x') = pickFreshName ctx (x, 0)
    in  "\\" ++ x' ++ "." ++ printTm ctx' t1 ++ ""
  TmApp _ t1 t2 -> "(" ++ printTm ctx t1 ++ " " ++ printTm ctx t2 ++ ")"
  TmVar _ x n ->
    if ctxLength ctx == n then indexToName ctx x else "[bad index]"

-- beta reduction
run :: Term -> Term
run (TmApp _i (TmAbs _i2 _vn func) arg') = replace arg' func
run app@(TmApp i f a) | app == napp = app
                      | otherwise   = run napp
  where napp = TmApp i (run f) a
run abs'@(TmAbs i vn f) | abs' == nabs = abs'
                        | otherwise    = run nabs
  where nabs = TmAbs i vn (run f)
run v = v
...
replace = replace' 0
 where
  ...
  replace' d arg (TmAbs i var_name func) =
    TmAbs i var_name (replace' (d + 1) arg func)
  replace' d arg (TmApp i arg' func) =
    TmApp i (replace' d arg arg') (replace' d arg func)
