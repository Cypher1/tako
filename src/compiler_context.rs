...
impl Compiler {
    ...
    fn manager<T: Task + 'static>(
        ...
        let manager = Arc::new(Mutex::new(TaskManager::<T>::new()));
        {
            ...
            tokio::spawn(async move {
                TaskManager::report_stats(manager, stats_requester, stats_sender);
            });
        }
        manager
    }

    fn with_manager<T: Task + 'static>(task_receiver: TaskReceiverFor<T>, manager: &Arc<Mutex<TaskManager<T>>>, result_sender: ResultSenderFor<T>) {
    ...
    pub fn lex(&self, path: PathBuf, contents: Option<String>, response_sender: ResultSenderFor<LexFileTask>) {
        ...
        if let Some(contents) = contents {
            tx.send(LexFileTask { path, contents });
        } else {
