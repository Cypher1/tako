...
fn app(inner: Term, arg: Term) -> Term {
    App{inner: Box::new(inner), arg: Box::new(arg)}
}

fn abs(inner: Term) -> Term {
    Abs{inner: Box::new(inner)}
}
...
mod test {
    ...
    fn eval_one_step() {
        let id = abs(var(0));
        let before = app(app(app(abs(abs(abs(app(app(var(1), var(2)), var(0))))), id.clone()), var(5)), var(6));
    ...
    fn eval_two_step() {
        let id = abs(var(0));
        let before = app(app(app(abs(abs(abs(app(app(var(1), var(2)), var(0))))), var(5)), id), var(6));
    ...
    #[test]
    fn eval_church_true() {
        assert_eq!(app(app(church_bool(true), church_nat(5)), church_nat(6)).eval(), church_nat(5));
    }

    #[test]
    fn eval_church_false() {
        assert_eq!(app(app(church_bool(false), church_nat(5)), church_nat(6)).eval(), church_nat(6));
    }

    #[test]
    fn eval_church_not_true() {
        assert_eq!(app(church_not(), church_bool(true)).eval(), church_bool(false));
    }

    #[test]
    fn eval_church_not_false() {
        assert_eq!(app(church_not(), church_bool(false)).eval(), church_bool(true));
    }
